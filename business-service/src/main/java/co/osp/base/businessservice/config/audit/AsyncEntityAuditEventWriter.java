package co.osp.base.businessservice.config.audit;

import co.osp.base.businessservice.entity.AbstractAuditingEntity;
import co.osp.base.businessservice.entity.EntityAuditEvent;
import co.osp.base.businessservice.constant.enumeration.EntityAction;
import co.osp.base.businessservice.constant.enumeration.EntityLabel;
import co.osp.base.businessservice.repository.EntityAuditEventRepository;
...
public class AsyncEntityAuditEventWriter {
    ...
    private EntityAuditEvent prepareAuditEntity(final Object entity, EntityAuditAction action) {
        ...
        auditedEntity.setEntityMessage(EntityAction.map.get(action.value())+ " "+ EntityLabel.map().get(entityClass.getName()));
        auditedEntity.setEntityLabel(EntityLabel.map().get(entityClass.getName()));
        Long entityId;
        String entityData;
        ...
        try {
            ...
            LinkedHashMap<String, Object> map = objectMapper.readValue(entityData, LinkedHashMap.class);
            if(map.get("files") != null){
                System.out.println("files existed");
                map.remove("files");
                entityData = objectMapper.writeValueAsString(map);
            }
